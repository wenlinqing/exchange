...
var socket = {
	...
	initWs() {
		...
		this.socket = new WebSocket('wss://darkchain.plus:8327/') 
		this.socket.onopen = () => {
			this.sendWsRequest( this.historyData )
			// this.sendWsRequest({
			...
			// })
			this.sendWsRequest({
				"sub": 'market.'+ this.historyData.symbol + '.kline.' + this.historyData.lintType,
			})
		}
	...
	message(resp) {
		...
		if (msg.tick) {
			EventTrad.emit('realTime', JSON.parse(msg.tick))
		}
		...
		if (msg && Array.isArray(msg)) {
			...
			EventTrad.emit('data', newArr.splice(-200))
			kLineData = newArr
		}
